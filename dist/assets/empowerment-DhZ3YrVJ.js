import{e as g,i as w,j as b}from"./main-Byi0pLCQ.js";let o=1;const f=20;let d=[];async function T(){console.log("📌 일반 매장 권한관리 초기화"),await y(),await i(),k(),$(),E()}async function y(){d=(await(await g("/model_admin_franchise?func=list-franchise")).json()).franchises??[];const c=document.getElementById("filterFranchise");c.innerHTML='<option value="">전체 프랜차이즈</option>',d.forEach(t=>{const e=document.createElement("option");e.value=t.franchiseId,e.textContent=t.name,c.appendChild(e)})}async function i(){const a=document.getElementById("searchKeyword").value.trim(),s=document.getElementById("filterFranchise").value;let e=(await(await g("/model_admin_user?func=get-admins")).json()).admins??[];const h=new Map;d.forEach(n=>{h.set(n.franchiseId,n.name)}),e=e.map(n=>({...n,franchiseName:n.franchiseId?h.get(n.franchiseId)??"-":"-"})),e=e.filter(n=>{var u;const r=!a||n.adminId.includes(a)||((u=n.franchiseName)==null?void 0:u.includes(a)),m=!s||n.franchiseId===s;return r&&m}),e=e.filter(n=>n.grade===4),e.sort((n,r)=>{const m=n.createdAt?new Date(n.createdAt).getTime():0;return(r.createdAt?new Date(r.createdAt).getTime():0)-m});const l=Math.ceil(Math.max(e.length,1)/f);o>l&&(o=l);const p=(o-1)*f,I=e.slice(p,p+f);v(I),L(o,l)}function v(a){const s=document.getElementById("store-table-body");s.innerHTML="";const c=t=>['<option value="">선택 없음</option>',...d.map(e=>`
