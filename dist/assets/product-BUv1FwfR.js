import{g as p,a as $,e as y}from"./main-B9HJQf1x.js";const a=[];function P(){const e=p();if(!e){window.showToast("❌ 사용자 정보가 없습니다.",3e3,"error");return}async function t(){return(await(await $(`/model_admin_menu?userId=${e==null?void 0:e.userId}&func=get-all-menu`)).json()).items}function n(o){const s=document.getElementById("menu-table-body");s&&(a.length=0,s.innerHTML=o.map(r=>{var u;const b=((u=r.image)==null?void 0:u.split("\\").pop())??"",k=encodeURIComponent(b),T=`https://model-narrow-road.s3.ap-northeast-2.amazonaws.com/model/${r.userId}/${k}`;return`
      <tr>
        <td >${r.no}</td>
        <td style="text-align: center;"><img src="${T}" alt="상품 이미지" style="width:36px;height:46px; object-fit:cover;display: inline-block;"></td>
        <td class="product-name whitespace-normal break-all text-sm" onclick="window.location.href='./product-detail.html?menuId=${r.menuId}'">
          ${r.name}
        </td>
