import{f}from"./main-RLPKnR3h.js";function I(){console.log("✅ register.ts 로드됨");const r=document.getElementById("register-form");if(!r){console.error("❌ 회원가입 폼을 찾을 수 없음");return}r.addEventListener("submit",async n=>{n.preventDefault();const e=document.getElementById("adminId").value.trim(),t=document.getElementById("password").value.trim(),s=document.getElementById("number").value.trim(),a=document.getElementById("confirmPassword").value.trim(),c=document.getElementById("privacyAgree").checked,m=document.getElementById("kakaoAgree").checked,u=`
