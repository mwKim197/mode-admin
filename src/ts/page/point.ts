...
async function getPointList() {
  ...
  const res = await apiGet(
    `/model_admin_mileage?userId=${userId}&func=mileage&limit=1000`
  );
    ...
    onPageChange: (pageData, page) => {
      ...
      pageData.forEach((item, index) => {
        ...
        deleteBtn.onclick = async () => {
          ...
          const res = await apiDelete(
            `/model_admin_mileage?userId=${userId}&func=mileage-delete&uniqueMileageNo=${item.uniqueMileageNo}`
          );
          if (!res.ok) {
            alert("❌ 삭제 실패");
            return;
  ...
  try {
    ...
    if (!res.ok) {
      try {
        ...
        const errorMessage =
          errorBody?.message ??
          `${mode === "create" ? "등록" : "수정"} 중 오류가 발생했습니다.`;
        alert(`❌ ${errorMessage}`);
      } catch (e) {
...
async function loadMileageHistory(point: PointItem, page = 1) {
  ...
  items.forEach((item: MileageHistoryItem, index: number) => {
    ...
    tr.innerHTML = `
            ...
            <td>${date.toLocaleTimeString("ko-KR", {
              hour12: false,
              hour: "2-digit",
              minute: "2-digit",
              second: "2-digit",
...
export function applyPhoneInputFormat(input: HTMLInputElement) {
  input.addEventListener("input", () => {
    ...
    if (value.length <= 3) {
      ...
      input.value = `${value.slice(0, 3)}-${value.slice(3, 7)}-${value.slice(
        7
      )}`;
    }
  });
