...
async function loadSalesSummary(user: any) {
    ...
    try {
        const res = await apiGet(`/model_payment?func=get-sales-summary&userId=${user.userId}`)
...
function startCounting() {
    ...
    counters.forEach((counter) => {
        const dataTarget = counter.getAttribute("data-target")?.replace(/,/g, "") || "0";
...
function getTagInfo(type: string) {
    switch (type) {
        case "emergency": return { label: "긴급", color: "red" };
        case "patch":     return { label: "패치", color: "blue" };
        case "event":     return { label: "이벤트", color: "org" };
        default:          return { label: "안내", color: "" };
    }
}
...
async function openPopup() {
    ...
    try {
        ...
        // 🔥 공지사항 API 호출
        const res = await apiGet(`/model_admin_notice?func=get-posts&startDate=${startDate}&endDate=${endDate}`);
...
function showPopup(notice: any): HTMLDivElement {
    ...
    document.getElementById("popupArea")!.style.backgroundColor = "rgba(0, 0, 0, 0.6)";
